...
namespace minips {
    ...
    void TestBgWorker() {
        ...
        SimpleRangeManager range_manager({{2, 4},
                                          {4, 7}}, {0, 1});
        ...
        std::thread worker_thread1(
                [kTestAppThreadId1, kTestModelId, &app_blocker, &downstream_queue, &range_manager, &ready]() {
                    ...
                    KVClientTable<float> table(kTestAppThreadId1, kTestModelId, &downstream_queue, &range_manager,
                                               &app_blocker);
                    // Add
        ...
        std::thread worker_thread2(
                [kTestAppThreadId2, kTestModelId, &app_blocker, &downstream_queue, &range_manager, &ready]() {
                    ...
                    KVClientTable<float> table(kTestAppThreadId2, kTestModelId, &downstream_queue, &range_manager,
                                               &app_blocker);
                    // Add
                    std::vector<Key> keys = {3, 4, 5, 6};
                    std::vector<float> vals = {0.1, 0.1, 0.1, 0.1};
